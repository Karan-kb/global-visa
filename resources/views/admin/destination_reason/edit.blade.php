...
@section('content')
    ...
    <section class="content-header">
        <h1>
            Destination Reason
            <small>Edit Destination Reason</small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        <div class="row">
            <div class="col-xs-12">
                ...
                {{-- @dd($reason) --}}
                <form action="{{ route('destination-reason.update', $reason->id) }}" method="POST" class="form-container"
                        ...
                        <select class="form-control" name="destination_id" required>
                            @foreach ($destinations as $destination)
                                <option value="{{ $destination->id }}"
                                    {{ old('destination_id', $reason->destination_id) == $destination->id ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="title" value="{{ $reason->title }}" required>
                    </div>
                    <div class="form-group">
                        <label for="description">Description*</label>
                        <input type="description" class="form-control" min="1" name="description"
                            value="{{ $reason->description }}" placeholder="Enter Description">
                    </div>
                        ...
                        <select class="form-control" id="iconSelect" name="icon" required>
                            ...
                            @foreach (config('svgs.icons') as $key => $icon)
                                <option value="{{ $key }}" data-icon="{{ $icon['icon'] }}"
                                    {{ $reason->image == $icon['icon'] ? 'selected' : '' }}>
                                    {{ $icon['title'] }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div id="iconPreview" class="mt-3" style="{{ $reason->image ? '' : 'display: none;' }} height:30px; width:40px;">
                        {{-- <label>Selected Icon Preview:</label> --}}
                        <div id="selectedIcon">{!! $reason->image !!}</div>
                    </div>
