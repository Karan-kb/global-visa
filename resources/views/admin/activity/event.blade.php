              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                @foreach($event as $event)
                    <tr>
                        ...
                        <td>{!! Str::limit($event->body, 200) !!}</td>
                        <td>
                            <a href="/event/{{$event->id}}/edit" style="border-radius:50%" class="btn btn-sm btn-warning"><i class="fa fa-edit"></i></a>
                         </td>
                         <td>   
                            <form method="post" action="/event/{{$event->id}}">
                              @csrf
                              @method('DELETE')
                                <button style="border-radius:50%" onclick="return confirm('Are You Sure...Do you want to delete this Event?')" class="btn btn btn-danger"> <i class="fa fa-trash"></i></button>
                              </form>
                        </td>
                    </tr>
                @endforeach
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h3 class="modal-title text-center" id="exampleModalLabel">Add Event</h3>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                <form action="{{route('event.store')}}" method="POST" class="form-container" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group">
                          ...
                          <input type="date" class="form-control" name="event_date" placeholder="Enter Location" required>
                      </div>
                      <div class="form-group">
                        <label for="time">Event Time*</label>
                        <input type="time" class="form-control" name="event_time" placeholder="Enter Location" required>
                    </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <label for="file">Select event Image</label>
