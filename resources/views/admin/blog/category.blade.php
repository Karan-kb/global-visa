...
@section('content')
    ...
    <section class="content-header">
      <h1>
      Category
        <small>Add/Modify Category</small>
      </h1>
      <ol class="breadcrumb">
              ...
              <table id="example1" class="table table-bordered table-striped">
                ...
                @foreach($cat as $cat)
                    <tr>
                        ...
                        <td>
                            <a href="/category/{{$cat->id}}/edit" style="border-radius:50%" class="btn btn-sm btn-warning"><i class="fa fa-edit"></i></a>
                          </td>
                          <td>  
                            <form method="post" action="/category/{{$cat->id}}">
                              @csrf
                              @method('DELETE')
                                <button style="border-radius:50%" onclick="return confirm('Are You Sure...Do you want to delete this Category?')" class="btn btn btn-danger"> <i class="fa fa-trash"></i></button>
                              </form>
                        </td>
                    </tr>
                @endforeach
...
<div class="modal fade" id="addMenuModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{route('category.store')}}" method="POST" class="form-container" enctype="multipart/form-data">
                        @csrf
